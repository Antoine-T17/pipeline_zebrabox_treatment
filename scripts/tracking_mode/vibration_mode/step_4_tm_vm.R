generate_figures_and_tables <- function(
    ...
    boxplot_list     = get("all_zone_combined_vibration_rest_boxplots", envir = .GlobalEnv),
    output_base_dir  = "outputs/tracking_mode/vibration_rest_mode",
  ...
  message("   • Generate PNG boxplots from 'all_zone_combined_vibration_rest_boxplots'.")
  ...
  boxplot_vibration_rest_mode <- get_input_local(
    "boxplot_vibration_rest_mode",
  ...
  if (boxplot_vibration_rest_mode == "pooled") {
    # Collect exactly two hex‐colors for Vibration vs. Rest periods
    boxplot_vibration_rest_periods_colors <- get_input_local(
      "boxplot_vibration_rest_periods_colors",
      "❓ Enter exactly 2 HTML hex colors (Vibration period, Rest period), comma-separated: ",
      validate_fn = function(raw) {
        cols <- trimws(unlist(strsplit(raw, ",")))
        length(cols) == 2 &&
          all(grepl("^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$", cols))
    ...
    names(boxplot_vibration_rest_periods_colors) <- c("Vibration period", "Rest period")
  }
  ...
  if (do_png) {
    figure_dirs <- c(
      ...
      file.path(output_base_dir, "figures/boxplots_vibration_rest/png/light_theme"),
      file.path(output_base_dir, "figures/boxplots_vibration_rest/png/dark_theme"),
  ...
  if (do_html) {
    figure_dirs <- c(
      ...
      file.path(output_base_dir, "figures/boxplots_vibration_rest/html/light_theme"),
      file.path(output_base_dir, "figures/boxplots_vibration_rest/html/dark_theme"),
  ...
  if (do_figures) {
    # --------------------
    # BOXPLOT VIBRATION REST GENERATION
    # --------------------
    message("-----")
    message("🔄 Generating vibration rest boxplots (Vibration period, Rest period)")
    message("-----")
    
    cap_text_vibration_rest_boxplots <- str_wrap("Each point corresponds to the mean of the response variable for one animal according to the selected period.",
                                             width = 60)
    ...
    for (var in names(boxplot_list)) {
      ...
      for (z in sort(unique(df$zone))) {
        ...
        for (th in c("light", "dark")) {
          ...
          if (boxplot_vibration_rest_mode == "separated") {
            ...
            if (do_png) {
              p_png <- ggplot(sub, aes(
                ...
                labs(
                  ...
                  caption = cap_text_vibration_rest_boxplots
                ) +
                theme_obj +
                theme(
            ...
            # pooled mode: side-by-side vibration/rest per condition
            ...
            if (do_png) {
              p_png <- ggplot(sub, aes(
                ...
                scale_fill_manual(values = boxplot_vibration_rest_periods_colors) +
                labs(
                  x       = "Condition",
                  y       = sprintf("%s (Zone %s)", var, z),
                  fill    = "Period",
    ...
    # --------------------
    message("-----")
    message("🔄 Generating cumulative boxplots (no difference between vibration and rest periods)")
    message("-----")
