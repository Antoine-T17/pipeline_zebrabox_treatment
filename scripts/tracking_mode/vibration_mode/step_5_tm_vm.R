process_zones <- function(enriched_data) {
  ...
  # Welcome message
  message("\n👋 Welcome to the Zone Processing Function!\n")
  ...
  message("💡 A graphical representation of the zones is available at 'inputs/tracking_mode/vibration_mode/docs/schema_well_zones.jpg'.")
  ...
  repeat {
    ...
    if (tolower(show_visual) %in% c("yes", "y", "no", "n")) {
      break
    } else {
  ...
  if (tolower(show_visual) %in% c("yes", "y")) {
  ...
  message("💾 Zone data has been saved in the global environment as 'zone_data_list'.\n")
