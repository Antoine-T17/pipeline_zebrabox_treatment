extract_data <- function() {
  ...
  message("- 📁 Your raw data file is saved in the 'inputs/tracking_mode/vibration_mode/raw_data' directory.")
  ...
  repeat {
    file_name <- readline(prompt = "📄 Enter the file name (including the extension, '.csv' or '.xlsx'): ")
    ...
    if (!grepl("\\.(csv|xlsx)$", file_name, ignore.case = TRUE)) {
      message("❌ The file name must include either '.csv' or '.xlsx' as the extension. Please try again.")
      next
    }
    
    # Construct the full path
    full_path <- file.path("inputs/tracking_mode/vibration_mode/raw_data", file_name)
