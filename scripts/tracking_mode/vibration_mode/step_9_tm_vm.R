generate_and_save_boxplots_with_excel_files <- function(input_data = get("pretreated_data_for_boxplots_df", envir = .GlobalEnv),
  ...
  # Introduction
  message("\n👋 Welcome to the Boxplot Generation Process!\n")
  ...
  # Step 3: Manage colors (only if user opted to generate boxplots)
  if (generate_boxplots) {
  ...
  if (generate_boxplots) {
    ...
    message("🎉 Boxplots generation completed in PNG, JPG, and HTML formats!\n")
  }
