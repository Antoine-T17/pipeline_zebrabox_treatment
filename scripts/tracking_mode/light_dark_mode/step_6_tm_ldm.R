...
#          reordering columns, and combining all zones.
#          Processes each plate's zone data separately.
...
calculate_and_clean_zone_data <- function(zone_data_list) {
  ...
  message("   • Remove rows whose 'start' time equals any specified removal times (per plate).")
  ...
  # Expect exactly one token per plate (tokens separated by ";")
  # Each token must be either "no" (case-insensitive) or a comma-separated list of positive numbers.
